...
export interface UploadService { login():Promise<UploadSession | void>; session():Promise<UploadSession>; logout():Promise<void>; upload(input:UploadInput,onProgress:(percent:number)=>void):Promise<BasketItem>; list():Promise<BasketItem[]>; edit(id:string,changes:{title?:string;caption?:string;isPublished?:boolean}):Promise<BasketItem>; archive(id:string):Promise<void> }
