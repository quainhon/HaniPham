...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    try {
      ...
      if (pathname.startsWith('/api/media') && request.method === 'GET') return await publicRoute(request, env);
      ...
      if (pathname === '/api/admin/logout' && request.method === 'POST') { requireOrigin(request, origin); return await logout(request, env); }
      if (pathname.startsWith('/api/admin/')) {
        if (request.method !== 'GET') requireOrigin(request, origin);
