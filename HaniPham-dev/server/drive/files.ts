...
// Import existing Drive files by explicit ID, without copying or deleting them.
// The configured folder must be the file's direct parent; drive.file access must
// already have been granted to this app for those files.
export async function inspectDriveFile(env: Env, id: string, folder: string, kind: 'audio'|'image') {
  if (!/^[A-Za-z0-9_-]{10,200}$/.test(id)) throw new HttpError(400, 'Invalid Drive file ID');
  const response = await fetch(`https://www.googleapis.com/drive/v3/files/${encodeURIComponent(id)}?fields=id,mimeType,size,parents,trashed`, {
    headers: { Authorization: `Bearer ${await driveToken(env)}` }
  });
  if (!response.ok) throw new HttpError(response.status === 404 ? 404 : 502, 'Drive file cannot be inspected');
  const info = await response.json() as {id?:string;mimeType?:string;size?:string;parents?:string[];trashed?:boolean};
  const types = kind === 'audio' ? MIME_AUDIO : MIME_IMAGE;
  const maximum = kind === 'audio' ? 100 * 1024 * 1024 : 25 * 1024 * 1024;
  if (info.trashed || !info.parents?.includes(folder) || !info.mimeType || !types.includes(info.mimeType) || !info.size || Number(info.size) <= 0 || Number(info.size) > maximum)
    throw new HttpError(400, 'Drive file is not in the approved folder or has an unsupported format/size');
  return info;
}
