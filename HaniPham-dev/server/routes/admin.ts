...
import { uploadFile, validateFile, driveToken, inspectDriveFile } from '../drive/files';
...
export async function adminRoute(request: Request, env: Env): Promise<Response> {
  ...
  // Index files already uploaded to the approved Drive folders (read-only in Drive).
  // Only the authorized owner can create the metadata record. Never trust a client
  // supplied ID, MIME, parent folder or a published status without validation.
  if (path === '/api/admin/import' && request.method === 'POST') {
    if (!(request.headers.get('Content-Type') ?? '').startsWith('application/json')) throw new HttpError(415, 'JSON required');
    const size = Number(request.headers.get('Content-Length') ?? 0);
    if (size > 8192) throw new HttpError(413, 'Metadata too large');
    const raw = await request.text();
    if (raw.length > 8192) throw new HttpError(413, 'Metadata too large');
    let data: Record<string, unknown>;
    try { data = JSON.parse(raw) as Record<string,unknown>; } catch { throw new HttpError(400, 'Invalid JSON'); }
    if (!data || typeof data !== 'object' || Array.isArray(data)) throw new HttpError(400, 'Invalid metadata');
    if (data.type !== 'song' && data.type !== 'photo') throw new HttpError(400, 'Invalid media type');
    ...
    const id = bounded(data.driveFileId, 'driveFileId', 200, true);
    const folder = requireConfig(type === 'song' ? env.DRIVE_SONGS_FOLDER_ID : env.DRIVE_PHOTOS_FOLDER_ID, 'Drive media folder');
    await inspectDriveFile(env, id, folder, type === 'song' ? 'audio' : 'image');
    let coverId: string | null = null;
    if (type === 'song') {
      coverId = bounded(data.coverDriveFileId, 'coverDriveFileId', 200, true);
      await inspectDriveFile(env, coverId, requireConfig(env.DRIVE_COVERS_FOLDER_ID, 'Cover folder'), 'image');
    }
    const record: RecordRow = { id: crypto.randomUUID(), type, title, artist, caption, tags: JSON.stringify(tags), mood: JSON.stringify(mood), duration_sec: null, published_at: new Date().toISOString(), is_published: Number(published), favorite_count: 0, play_count: 0, drive_file_id: id, cover_drive_file_id: coverId, deleted_at: null };
    const existing = await db.prepare('SELECT id FROM media WHERE drive_file_id = ? AND deleted_at IS NULL').bind(id).first<{id:string}>();
    if (existing) throw new HttpError(409, 'This Drive file is already indexed');
    await insertRow(db, record);
  ...
  if (!Number.isFinite(length) || length <= 0 || length > 34 * 1024 * 1024) throw new HttpError(413, 'Upload requires a valid Content-Length up to 34 MiB');
